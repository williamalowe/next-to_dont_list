      ...
      <ul className="mt-4 flex flex-col gap-y-2">
        ...
        <li key={dont._id} className="ml-6 list-disc">
          <div className="relative flex items-center gap-x-4">
            <p className="text-4xl px-4 bg-orange-300 rounded-full">{dont.task} <span className={`${dont.score < 0 ? 'text-red-600' : dont.score > 0 ? 'text-green-600' : 'text-black/50'} font-bold ml-auto`}>{dont.score}</span></p>
